...
extern "C" fn trampoline<const OP: u8>(data: *mut u8) {
    ...
    // borrow_mut is held across both the callback and the forward below: a
    // re-entrant dispatch of this opcode from inside either (e.g. via
    // chat_print -> ChatEvents -> Protocol roundtrip) hits the held borrow and
    // panics with BorrowMutError. The crash is deliberate -- re-entrancy is a
    // consumer bug to fix, not something this layer silently absorbs.
    CALLBACK.with(|cb| {
        let mut slot = cb[OP as usize].borrow_mut();
        let should_suppress = slot.as_mut().is_some_and(|f| f(bytes));
        if !should_suppress {
            OLD.with(|a| {
...
/// `BorrowMutError`. The crash is deliberate -- fix the re-entrancy in your
/// callback rather than relying on this layer to absorb it.
///
